...
def t_INTEGER(t): # Se procesa el string que represente un numero aqui.
    r'\d+'
    t.value=int(t.value)
    if t.value<=sys.maxint and t.value>=-2**31:
        return t
    else:
        msj_err= "Error: This token will produce overflow \'%s\' at line %d, column %d"
        msj_err= msj_err % (t.value,t.lexer.lineno,find_column(t.lexer.lexdata,t))
...
def t_error(t): # Los simbolos que no esten contemplados en el lenguaje generan error
    msj_err= "Error: invalid token \'%s\' at line %d,column %d"
