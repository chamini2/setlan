...
class Table:
	...
	def insert(self, nombre, tipo, reservado = False):
		if (not self.contains(nombre, True)):
			...
			if self.table[nombre].getType() == 'int':
				self.table[nombre].update(0)
			elif self.table[nombre].getType() == 'set':
				self.table[nombre].update([])
			elif self.table[nombre].getType() == 'bool':
				self.table[nombre].update("false")
			return True
		else: 
			return False 
