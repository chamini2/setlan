...
# Gramatica auxiliar para los identificadores
def p_Aux_ID(p):
	'''aux_id : id aux_id
						| Coma id
						| epsilon'''
...
def p_Asignacion_Conj(p):
	'''Asignacion_Conj : id Equal Llave_Abre ListaNumeros Llave_Cierra'''
	p[0] = Clase.Asignacion_Conj(p[1],p[4])
...
def p_Instruccion2(p):
	'''Instruccion2 : Instruccion2 Instruccion2_Base
									| Instruccion2_Base'''
	if len(p) == 2:
		p[0] = []
		p[0].append(p[1])
...
def p_Lista_Declaracion_Base(p):
	'''Lista_Declaracion_Base : Type aux_id Pto_Coma
...
def p_Bloque(p):
	'''Bloque : Llave_Abre Declaracion Instruccion2 Llave_Cierra'''
...
def p_Ciclo_Repeat1(p):
	'''Ciclo_Repeat1 : Repeat Instruccion1 While Par_Abre Expresion Par_Cierra Do Instruccion1 Pto_Coma'''
	p[0] = Clase.Expre_Repeat1(p[2],p[4],p[6])
...
def p_Ciclo_Repeat3(p):
	'''Ciclo_Repeat3 : Repeat Bloque While Par_Abre Expresion Par_Cierra Pto_Coma'''
...
precedence = (
	('nonassoc', 'Equal','Equiv','NotEquiv','Greater','GreaterEqual','Less','LessEqual',\
							 'NumElemConj','IsInConj'),	
	('left','Or'),
	('left','And'),
	('left','Not'),
	('left','MultConj','DivConj','ModConj','MaxConj','MinConj'),
	('left','SumaConj','RestaConj'),
	('left','Suma','Resta'),
	('left','Mult','Div','Mod'),
	('left','Union','Inter','Dif')
)
